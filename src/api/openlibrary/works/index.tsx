...
export default class WorksApi {
  private static instance: WorksApi;

  private basePath?: string = undefined;

  static getInstance(basePath?: string): WorksApi {
    if (!WorksApi.instance) {
      WorksApi.instance = new WorksApi();

      if (!basePath) {
        WorksApi.instance.basePath = OPEN_LIBRARY_URL;
      }
    }

    return WorksApi.instance;
  }
  ...
  async getWorksBySubject(
    ...
    if (response.status >= 200 && response.status < 300) {
      const jsonResponse: APIWorksBySubjectRawResponse = await response.json();
